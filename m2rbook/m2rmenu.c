/* m2rmenu.c is the menu processor for mif2go
 * Copyright 2004 Omni Systems, Inc.  All Rights Reserved.
 */
...
VoidT CreateFMMenu(VoidT)
{
	...
	F_ObjHandleT washFileCmdId;
	F_ObjHandleT washBookCmdId;
	F_ObjHandleT washBookAllCmdId;
	...
	washFileCmdId = F_ApiDefineAndAddCommand(WASH_FILE, fileMenuId,
									"WashMIFFile", WashFileStr, "");
	F_ApiSetInt(FV_SessionId, washFileCmdId, FP_EnabledWhen,
							 FV_ENABLE_NEEDS_DOCP_ONLY);
	F_ApiSetId(fileMenuId, washFileCmdId, FP_PrevMenuItemInMenu, exportFileCmdId);
	F_ApiAddCommandToMenu(qFileMenuId, washFileCmdId);
	F_ApiSetId(qFileMenuId, washFileCmdId, FP_PrevMenuItemInMenu, exportFileCmdId);
	...
	washBookCmdId = F_ApiDefineAndAddCommand(WASH_BOOK, bookFileMenuId,
									"WashMIFBook", WashFileStr, "");
	washBookAllCmdId = F_ApiDefineCommand(WASH_BOOK_ALL, 
									   "WashMIFBookAll", WashAllFileStr, "");
	F_ApiSetInt(FV_SessionId, washBookCmdId, FP_EnabledWhen,
							 FV_ENABLE_NEEDS_BOOKP_ONLY);
	F_ApiSetId(bookFileMenuId, washBookCmdId, FP_PrevMenuItemInMenu, exportBookCmdId);
	F_ApiSetInt(bookFileMenuId, washBookCmdId, FP_HasShiftOrUnshiftCommand,
	             FV_ITEM_HAS_SHIFT_COMMAND);
	F_ApiSetId(bookFileMenuId, washBookCmdId, FP_ShiftOrUnshiftCommand, washBookAllCmdId);
	F_ApiAddCommandToMenu(qBookFileMenuId, washBookCmdId);
	F_ApiSetId(qBookFileMenuId, washBookCmdId, FP_PrevMenuItemInMenu, exportBookCmdId);
	F_ApiSetInt(qBookFileMenuId, washBookCmdId, FP_HasShiftOrUnshiftCommand,
	             FV_ITEM_HAS_SHIFT_COMMAND);
	F_ApiSetId(qBookFileMenuId, washBookCmdId, FP_ShiftOrUnshiftCommand, washBookAllCmdId);
